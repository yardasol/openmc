...
public:
  ...
  double compute_sare();    // Compute the SARE (Source Averaged Relative Error)
  bool is_sare_converged(); // Determing if the solution is converged
...
private:
  ...
  // Flag to track if the solution has converged
...
void increment_batches(
  bool inactive);   // Incremenet the number of batches and related
