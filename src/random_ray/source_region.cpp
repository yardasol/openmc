...
SourceRegionHandle::SourceRegionHandle(SourceRegion& sr)
  : negroups_(sr.scalar_flux_old_.size()), material_(&sr.material_),
    ...
    source_final_(sr.source_final_.data()), external_source_(sr.external_source_.data()),
    ...
    source_bd_(sr.source_bd_.data()),
    precursors_bd_(sr.precursors_bd_.data()),
...
SourceRegionHandle SourceRegionContainer::get_source_region_handle(int64_t sr)
{
  ...
  if (settings::run_mode == RunMode::TIME_DEPENDENT ||
    ...
    if (RandomRay::time_method_ == RandomRayTimeMethod::SDP) {
      ...
      handle.scalar_flux_time_derivative_2_ = &scalar_flux_time_derivative_2(sr, 0);
