...
#pragma omp parallel for reduction(+ : n_hits)
  for (int64_t sr = 0; sr < n_source_regions(); sr++) {
    ...
    for (int g = 0; g < negroups_; g++) {
      ...
      // Also check TD flux
      if (settings::is_initial_condition || settings::run_mode == RunMode::TIME_DEPENDENT){
...
double FlatSourceDomain::compute_fixed_source_normalization_factor() const
{
  ...
  if (settings::run_mode == RunMode::EIGENVALUE ||
    ...
    // neutron produced).
    // TODO: check if a delayed fission rate needed to normalize the precursor population...
    // need to run a test problem using mc tallies?
...
#pragma omp atomic
    ...
    if (settings::run_mode == RunMode::TIME_DEPENDENT ||
      ...
      for (int dg = 0; dg < ndgroups_; dg++) {
	    ...
	    // TODO: check if multiplying by source normalization factor is the right thing to do now...
